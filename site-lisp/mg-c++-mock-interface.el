...
(defun mg-remove-default-parameters ()
  (if (save-excursion
        (let ((matching-bracket (mg-find-matching-bracket)))
          (let ((found-default-parameter (re-search-forward "=" matching-bracket t 1)))
            (if found-default-parameter
                (let ((begin (1- (point))))
                  (re-search-forward "[^[:blank:]]+" matching-bracket nil 1)
